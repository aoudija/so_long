...
/*   Updated: 2023/02/10 09:11:49 by aoudija          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ecape_norm(char *map, int hold_p, int hold_n, int count)
{
	int	i;

	i = -1;
	count = morty_is_here(map, count, hold_p - hold_n - 1);
...
int	player_up(char *map, int count)
{
	...
	while (map[++hold_n] != '\n')
		;
	count = ecape_norm(map, hold_p, hold_n, count);
	return (count);
}

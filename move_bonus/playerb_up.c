...
/*   Updated: 2023/02/10 13:30:46 by aoudija          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ecape_norm(t_data *data, int hold_p, int hold_n, int count)
{
	int	i;

	i = -1;
	count = morty_is_here(data, count, hold_p - hold_n - 1);
...
int	playerb_up(t_data *data, int count)
{
	...
	while (data->map[++hold_n] != '\n')
		;
	count = ecape_norm(data, hold_p, hold_n, count);
	return (count);
}

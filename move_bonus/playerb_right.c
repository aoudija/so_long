...
/*   Updated: 2023/02/10 13:29:06 by aoudija          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	escape_normy(t_data *data, int hold_p, int count)
{
	int	i;
	...
	while (data->map[++i])
	{
		if (i == hold_p + 1 && (data->map[hold_p + 1] == 'E'
				|| data->map[hold_p + 1] == 'F' || data->map[hold_p + 1] == 'T')
			&& pickle_exist(data->map))
			data->map[hold_p] = 'P';
...
int	playerb_right(t_data *data, int count)
{
	...
	count = escape_normy(data, hold_p, count);
	return (count);
}
